...
int main(){
    ...
    double gX, gY, dX, dY; cin >> gX >> gY >> dX >> dY;
    
    double escapeX, escapeY;

    while(cin >> escapeX >> escapeY){
        double gDist = sqrt(pow((gX - escapeX), 2) + pow((gY - escapeY), 2));
        double dDist = sqrt((pow((dX - escapeX), 2) + pow((dY - escapeY), 2))) / 2;
        if( gDist <= dDist){
            co "The gopher can escape through the hole at (";
            co fixed << setprecision(3) << escapeX << "," << escapeY;
            co ")." endl;
            exit(0);
        }
    }

    co "The gopher cannot escape." endl;
